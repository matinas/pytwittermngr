...
1. **Create management lists**: creates all the lists used for managing (MNG.Main, MNG.Unfowllowed, MNG.Unfollowed.Back, MNG.Unfowllowed.Me) in the authenticated user's account. Check the description of each list at the beginning of this readme file. Use this option in case you don't want to creat the lists manually through the Twitter inferface.
...
3. **Process unfollows in main management list**: processes the MNG.Main list in search of user's that have unfriended the authenticated users. Unfriends those users and moves them to the MNG.Unfollowed.Me list. Additionally, it finds user's which the authenticated user is not following anymore and moves them to the MNG.Unfollowed list
...
6. **Unfollows stats**: shows stats related to the amount of users that unfollowed the authenticated user for no reason
